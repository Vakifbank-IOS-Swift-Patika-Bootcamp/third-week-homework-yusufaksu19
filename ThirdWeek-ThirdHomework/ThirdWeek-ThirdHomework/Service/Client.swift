...
final class Client {
  ...
  enum Endpoints {
    static let base = "https://programming-quotes-api.herokuapp.com/Quotes/random"
  ...
  class func getQuotes(completion: @escaping (QuoteModel?, Error?) -> Void) {
    taskForGETRequest(url: Endpoints.quotes.url, responseType: QuoteResponseModel.self) { response, error in
